...
int main(int argc, char **argv) {
	...
	int c;
	std::string directory_s, amber_file_s, pdb_file_s, vmd_s, namd_s;
	while ( (c = getopt(argc, argv, "f:g:s:t:")) != -1 ) {

		switch(c) {
			case 'd':
				directory_s.assign(optarg);
				break;
			case 'a':
	...
	if (directory_s.size() == 0) {
		std::cout << "Scratch directory required. Use -d <directory>." << std::endl;
		exit(EXIT_FAILURE);
	}
	...
	if (vmd_s.size() == 0) {
		std::cout << "VMD command required. Use -v <command>." << std::endl;
		exit(EXIT_FAILURE);
	}

	if (namd_s.size() == 0) {
		std::cout << "NAMD2 command required. Use -n <command>." << std::endl;
		exit(EXIT_FAILURE);
	}
